defmodule TrialAppWeb.UserAuthTest do
  use TrialAppWeb.ConnCase, async: true
  ...
  import TrialApp.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, TrialAppWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      TrialAppWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        TrialAppWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
