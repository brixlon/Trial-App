defmodule TrialAppWeb.AttacheeProfileLive do
  ...
  defp mark_and_update_report(socket, report) do
    if is_nil(report.viewed_at) do
      # 1. Update Database
      {:ok, updated_report} = Reports.mark_as_viewed(report, DateTime.utc_now() |> DateTime.truncate(:second))

      # 2. Update the main list so the badge changes instantly from "New" to "Viewed"
      updated_list = Enum.map(socket.assigns.reports, fn r ->
        if r.id == updated_report.id, do: updated_report, else: r
      end)

      # 3. Update selected_report if it's currently open
      socket =
        if socket.assigns.selected_report && socket.assigns.selected_report.id == updated_report.id do
            ...
            <div class="bg-white shadow rounded-xl p-6">
              ...
              <%= if Enum.empty?(@reports) do %>
                <div class="text-center py-12">
                  ...
                  <p class="text-sm text-gray-400">Your supervisor will send evaluation reports here</p>
                </div>
              <% else %>
                <div class="space-y-3">
                      ...
                      <div class="flex items-center justify-between">
                        <div class="flex-1">
                          ...
                          <div class="mt-1 text-sm text-gray-600">
                            <span>Period: <%= format_date(report.period_start) %> - <%= format_date(report.period_end) %></span>
                ...
                <dl class="space-y-3">
                  <div class="flex justify-between">
                    ...
                    <dd class="text-sm text-gray-900">
                      <%= format_date(@selected_report.period_start) %> - <%= format_date(@selected_report.period_end) %>
                    </dd>
                  </div>
                  <div class="flex justify-between">
