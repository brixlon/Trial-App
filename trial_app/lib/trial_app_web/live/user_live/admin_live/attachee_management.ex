defmodule TrialAppWeb.AdminLive.AttacheeManagement do
  ...
  def handle_event("edit_attachee", %{"id" => id}, socket) do
    attachee =
      Eams.get_attachee!(id) |> Repo.preload([:user, :organization, :department, :programs])

    # Load departments for the organization
    departments =
      if attachee.organization_id do
  ...
  def handle_event("save", %{"attachee" => params}, socket) do
    ...
    result =
      if socket.assigns.editing_attachee do
    ...
    case result do
      {:ok, attachee} ->
        ...
        {:noreply,
         ...
         |> put_flash(
           :info,
           if(socket.assigns.editing_attachee,
             do: "Attachee updated successfully!",
             else: "Attachee created successfully!"
           )
         )}
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <.input
                        ...
                        label="Organization"
                        options={[
                          {"Select Organization", ""} | Enum.map(@organizations, &{&1.name, &1.id})
                        ]}
