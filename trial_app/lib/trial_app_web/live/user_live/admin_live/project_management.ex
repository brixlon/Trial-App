defmodule TrialAppWeb.AdminLive.ProjectManagement do
  ...
  def mount(_params, _session, socket) do
    {:ok,
     ...
     |> assign(:form_data, %{"name" => "", "description" => "", "organization_id" => "", "department_id" => "", "program_id" => "", "supervisor_id" => "", "code" => "", "starts_on" => "", "ends_on" => ""})
     |> assign(:errors, %{})}
  end
  ...
  def render(assigns) do
    ...
    <div class="ml-64 p-8">
      ...
      <div class="flex justify-end mb-3">
        <button phx-click="new" class="px-4 py-2 rounded-lg bg-indigo-600 text-white">New Project</button>
      </div>
            ...
            <.form for={to_form(@form_data, as: :project)} id="project-form" phx-submit="save" phx-change="update">
              <div class="space-y-4">
                <div>
                  ...
                  <input name="name" value={@form_data["name"]} class="w-full border rounded p-2" required />
                </div>
                ...
                <div>
                  ...
                  <input name="code" value={@form_data["code"]} class="w-full border rounded p-2" />
                </div>
                <div>
                  <label class="block text-sm font-medium">Supervisor</label>
                  <select name="supervisor_id" class="w-full border rounded p-2" required>
                    <option value="">Select supervisor</option>
                    <%= for s <- @supervisors do %>
                      <option value={s.id} selected={to_string(s.id) == @form_data["supervisor_id"]}>{s.username || s.email}</option>
                    <% end %>
                  </select>
                </div>
                <div>
