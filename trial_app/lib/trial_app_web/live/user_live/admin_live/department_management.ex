defmodule TrialAppWeb.AdminLive.DepartmentManagement do
          ...
          <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl transform transition-all">
            ...
            <form phx-submit="save_department" phx-change="update_form" class="p-6 space-y-6">
              ...
              <div>
                ...
                <p class="mt-2 text-sm text-gray-500">Optional: Add a description to help identify this department</p>
              </div>
  ...
  def handle_event("save_department", params, socket) do
    ...
    errors = %{}
    errors = if String.trim(name) == "", do: Map.put(errors, :name, "Department name is required"), else: errors

    if map_size(errors) == 0 do
      if socket.assigns.editing_id do
        # Update existing department
        updated_departments = Enum.map(socket.assigns.departments, fn dept ->
        ...
        {:noreply,
          socket
          |> assign(show_form: false, editing_id: nil, form_data: %{name: "", description: ""}, errors: %{})
