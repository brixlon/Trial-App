defmodule TrialAppWeb.AdminLive.DepartmentManagement do
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     ...
     |> assign(:form, to_form(Department.changeset(%Department{}, %{})))
     |> assign(:search_query, "")
          ...
          <div class="max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-6 text-white shadow-lg">
                <div class="flex items-center justify-between">
                  <div>
                    <p class="text-purple-100 text-sm font-semibold">Organizations</p>
                  ...
                  <%= if Enum.empty?(@organizations) do %>
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                      <p class="text-yellow-800 text-sm mb-3">
                        ⚠️ You need to create organizations before adding departments.
                      </p>
                      <.link
                  ...
                  <div class="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all border-2 border-gray-100 hover:border-indigo-200 p-6 group">
                    ...
                    <div class="flex items-center gap-4 text-sm text-gray-500 pt-4 border-t border-gray-100">
                      ...
                      <div class="flex items-center gap-1">
                        ...
                        <span><%= length(department.teams) %> Teams</span>
                      </div>
                    </div>
                  </div>
                <% end %>
      ...
      <%= if @show_form do %>
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl transform transition-all">
            ...
            <!-- Modal Body -->
            <.form for={@form} phx-submit="save_department" phx-change="validate_department" class="p-6 space-y-6">
              <.input
                field={@form[:name]}
                type="text"
  ...
  def handle_event("new_department", _params, socket) do
    {:noreply, assign(socket,
      show_form: true,
      editing_id: nil,
      form: to_form(Department.changeset(%Department{}, %{}))
    )}
  end
  ...
  def handle_event("validate_department", %{"department" => dept_params}, socket) do
    changeset = Department.changeset(%Department{}, dept_params)
    {:noreply, assign(socket, form: to_form(changeset))}
  end

  def handle_event("save_department", %{"department" => dept_params}, socket) do
    if socket.assigns.editing_id do
      department = Organizations.get_department!(socket.assigns.editing_id)
      case Organizations.update_department(department, dept_params) do
        {:ok, _dept} ->
          departments = Organizations.list_all_departments()
          {:noreply,
            socket
            |> assign(show_form: false, editing_id: nil, form: to_form(Department.changeset(%Department{}, %{})))
            |> assign(departments: departments)
      ...
      case Organizations.create_department(dept_params) do
        {:ok, _dept} ->
          departments = Organizations.list_all_departments()
          {:noreply,
            socket
            |> assign(show_form: false, form: to_form(Department.changeset(%Department{}, %{})))
            |> assign(departments: departments)
  ...
  def handle_event("confirm_delete", %{"id" => id}, socket) do
    department = Organizations.get_department!(String.to_integer(id))
    case Organizations.delete_department(department) do
      {:ok, _dept} ->
        departments = Organizations.list_all_departments()
        {:noreply,
          socket
          |> assign(departments: departments, show_delete_modal: false, deleting_id: nil)
